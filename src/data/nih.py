...
def build_nih_manifest(nih_dir: Path) -> pd.DataFrame:
    """Builds a manifest (path, label, + age/sex/view for the later
    mechanism/association analysis) from the NIH Data_Entry CSV,
    harmonized to Kaggle's binary pneumonia scheme. Never used for
    training, external validation only.
    """
    ...
    columns = ["path", "label", "Patient Age", "Patient Sex", "View Position"]
    if "Patient ID" in harmonized.columns:
        columns.append("Patient ID")
    return harmonized[columns].reset_index(drop=True)
